...
public class LocalController {
	...
	public boolean alterar(Local local) {
    	Connection con = Conexao.conectar();
    	String sql = "update tb_local set nome=?, rua=?, numero=?,bairro=?,cidade=?,estado=?,cep=? where id = ?";
    	try {
	...
	public boolean salvar(Local local) {
    	Connection con = Conexao.conectar();
    	String sql = "insert into tb_local(nome,rua,numero,bairro,cidade,estado,cep)values(?,?,?,?,?,?,?)";
    	try {
			PreparedStatement stm = con.prepareStatement(sql);
			stm.setString(1, local.getNome());
			stm.setString(2, local.getRua());
	...
	public List<Local> getLocais(){
    	...
    	Connection con = Conexao.conectar();
    	try {
			PreparedStatement stm = con.prepareStatement("select * from tb_local");
			ResultSet rs = stm.executeQuery();
			while (rs.next()) {
